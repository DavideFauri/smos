...
spec = do
  ...
  goldenFormatsSpec
    ...
    ( makeSmosFile
        [ Node
            ( (newEntry "clocked in")
                { entryLogbook = LogOpen (UTCTime (fromGregorian 2022 01 22) 1234.567) []
                }
            )
            [],
          Node
            ( (newEntry "clocked out")
                { entryLogbook =
                    LogClosed
                      [ LogbookEntry
                          { logbookEntryStart = UTCTime (fromGregorian 2022 01 22) 1234.567,
                            logbookEntryEnd = UTCTime (fromGregorian 2022 01 22) 1234.567
                          }
                      ]
                }
            )
